 ...
 const createItem = (product, templateElement) => {
   ...
   if (product.comp!=null){
        ...
        if (product.year!=null){
            ...
            years=product.year.split(",").filter(onlyUnique);
        }else{
            year.textContent[0] = "Universale";
            years="Universale";
        }
        makes=product.make.split(",").filter(onlyUnique);
        models=product.model.split(",").filter(onlyUnique);
    }

   if (make) make.textContent = product.make.split(",").filter(onlyUnique);
   if (model) model.textContent = product.model.split(",").filter(onlyUnique);
   if (producer) producer.textContent = product.brand;
   if (product.desc != null){
        if(product.desc-full!=null){
